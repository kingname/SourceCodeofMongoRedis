...
class MongoUtil(object):
    ...
    def query_question(self, question_id_score_list: List[Tuple[bytes, int]]) -> (dict, int):
        ...
        :return:
        """
        total_question = self.question.find().count()
        id_score_dict = {}
        id_order_dict = {}
        object_id_list = []
    ...
    def query_answer(self, question_id: str, answer_id_score_list: List[Tuple[bytes, int]]) -> dict:
        ...
        question = self.question.find_one({'_id': ObjectId(question_id)})
        question_answer_dict = {
            'question_id': str(question['_id']),
            'question_title': question['title'],
            'question_detail': question['detail'].split('\n'),
            'question_author': question['author'],
            'answer_num': self.answer.find({'question_id': ObjectId(question_id)}).count()
        }
        answers = self.answer.find({'_id': {'$in': object_id_list}})
        answer_list = []
